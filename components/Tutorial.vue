...
<template>
  <client-only>
  <div >
    <div class="w-full h-full min-h-screen sm:px-6 lg:px-8">
      <button class="rounded-full mb-4 mt-4 bg-blue-400 p-2 px-4 text-white" @click="createPdf" >Create pdf</button>
      ...
      <div ref="pdf" class="h-full align-middle tracking-normal  bg-gray-100 sm:pt-0 font-sans items-center min-h-screen sm:items-center justify-center overflow-hidden shadow sm:rounded-lg">
    <Header v-bind="getCV.header"/>
        <div class="p-6">
        <div class="grid grid-cols-3 sm:grid-cols-4 gap-6 ">
          <Skills v-bind="getCV.skills"></Skills>
          <div class="col-span-3">
            <!-- NAME CARD -->
            <ContactInfo v-bind="getCV.contact"/>
            <Experience v-for="experience of getCV.experiences" v-bind="experience" />
